...
% if selecting for a particular training pattern, remove data for alternate
% pattern:
if strcmp(pattern,'UpBar')==1 || strcmp(pattern,'DnBar')==1
    ...
    iremoveL  = find(barL<0.5);
    iremoveNL = find(barNL<0.5);
    PIL( iremoveL, :) = [];
    PINL(iremoveNL,:) = [];
    puffedL( iremoveL, :) = [];
    puffedNL(iremoveNL,:) = [];
...
elseif strcmp(type,'kir')
    title({['p_{Kir} 2-8 = ',num2str(p28NL)];['p_{Par} 2-8 = ',num2str(p28L)];['p_{Par/Kir} 2-2 = ',num2str(p22)];['p_{Par/Kir} 8-8 = ',num2str(p88)]});
else
    error('unrecognized data type')
end
set(gca,'fontsize',16)
